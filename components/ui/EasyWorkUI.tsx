...
import { useSession, signOut } from 'next-auth/react'; // ✅ 加這行
...
export default function EasyWorkUI() {
  const { data: session } = useSession(); // ✅ 加這行
  ...
  const fetchData = async (endpoint: string, key: string, body: any = form) => {
    const setState = (text: string) => setResults((r) => ({ ...r, [key]: text }));
    setLoading(true);
    try {
      ...
      setState(data.outline || data.draft || data.feedback || data.result || '');
    } catch (err: any) {
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1">
        <div className="w-64 border-r p-4 bg-gray-50 overflow-y-auto">
          ...
          <Button onClick={() => fetchData('/api/outline', 'outline')} isLoading={loading} className="w-full bg-blue-500 text-white mt-2">🧠 產生大綱</Button>
          <Button onClick={() => fetchData('/api/draft', 'draft', { ...form, outline: results.outline })} isLoading={loading} className="w-full bg-blue-500 text-white mt-2">✍️ 草稿產生</Button>
            ...
            {sections.map(({ key, label }) => (
              <TabsContent key={key} value={key}>
                <Card className="p-4 mt-4 bg-gray-50 relative">
                  ...
                  {copied[key] && (
                    <span className="absolute bottom-2 right-20 text-green-500 text-sm">✅ 已複製！</span>
                  )}
                </Card>
              </TabsContent>
            ))}
